...
def test_historical_security_query_includes_later_delisted_stock(tmp_path):
    ...
    available = datetime(2023, 1, 3, tzinfo=timezone.utc)
    assert repo.list_effective_symbols(date(2023, 1, 3), available) == ["600001", "600002"]
    assert repo.list_effective_symbols(date(2025, 1, 3), available) == ["600002"]
...
def test_daily_bar_query_cannot_return_future_rows(tmp_path):
    ...
    rows = repo.get_daily_bars(["600002"], end=date(2025, 1, 2), available_before=available)
    assert [row["trade_date"] for row in rows] == [date(2025, 1, 2)]


def test_future_available_at_rows_are_excluded(tmp_path):
    repo = MarketDataRepository(tmp_path / "market.duckdb")
    repo.upsert_daily_bars([
        {"symbol": "600002", "trade_date": date(2025, 1, 2), "close": 10.0,
         "open": 9.8, "high": 10.2, "low": 9.7, "volume": 1000,
        ...
        {"symbol": "600002", "trade_date": date(2025, 1, 3), "close": 11.0,
         "open": 10.0, "high": 11.2, "low": 9.9, "volume": 1200,
         "amount": 12000.0, "available_at": datetime(2099, 1, 1, tzinfo=timezone.utc),
         "source": "fixture"},
    ])
    available = datetime(2025, 1, 5, tzinfo=timezone.utc)
    rows = repo.get_daily_bars(["600002"], end=date(2025, 1, 5), available_before=available)
    assert len(rows) == 1
