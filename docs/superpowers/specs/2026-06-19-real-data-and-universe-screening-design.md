...
> 日期：2026-06-19
>
> 状态：用户已确认；2026-06-19 架构修订（纠正 Tushare 默认主源漂移）
>
...
1. 建立统一、可插拔的真实数据 Provider，**默认实现为 `FreeAStockProvider`（免费公开源组合）**；Tushare Pro、聚宽等仅作可选付费增强适配器。
...
```text
FixtureProvider ────┐
FreeAStockProvider ─┼→ MarketDataRepository → 现有 Pipeline
TushareProvider ────┘   （可选，显式启用）
```
...
### 4.1 架构原则（与 auto-stock-screening-design 对齐）

1. **免费源为默认**：未设置 `TUSHARE_TOKEN` 时，`init`、同步、盘后调度必须可运行。
2. **付费源为可选增强**：Tushare、聚宽等只能通过可选依赖和显式配置启用。
3. **禁止静默切换**：不得因 Token 缺失自动回退或拼接多源；每条记录保留 `source` 与 `pit_level`。
4. **PIT 诚实**：免费源不具备历史 PIT 的数据集必须标记 `CURRENT_ONLY` 或拒绝进入正式历史回测；**禁止伪造 `available_at` 或历史板块成员**。
5. **复用现有能力**：`FreeAStockProvider` 封装 `tradingagents/dataflows/a_stock.py` 已验证的 mootdx / 东财 / 新浪 / 腾讯等直连 HTTP，不重新引入 akshare。

权威能力矩阵：`docs/data/data-capability-matrix.yaml`（version 2）。

### 4.2 Provider 选择

```yaml
# ~/.tradingagents/config.yaml（示例）
market_data:
  provider: free   # free | tushare | fixture
```

环境变量：`TRADINGAGENTS_MARKET_DATA_PROVIDER=free|tushare|fixture`

优先级（高 → 低）：

1. CLI `--provider`
2. 环境变量 `TRADINGAGENTS_MARKET_DATA_PROVIDER`
...
| Provider | 行为 |
|----------|------|
| `free` | 不得读取或要求 `TUSHARE_TOKEN`；probe 只探测免费端点 |
| `tushare` | 必须显式选择；缺 Token 或可选依赖 → 立即报错，**不**自动回退 free |
| `fixture` | 仅测试、CI 和显式 `--fixture` 离线运行 |

安装：

```bash
pip install -e ".[screener]"                    # 默认含 FreeAStockProvider
pip install -e ".[screener,screener-tushare]"   # 可选 Tushare
```

### 4.3 候选方案

#### 方案 A（默认）：免费组合源 — `FreeAStockProvider`

| 数据集 | 免费源 | 默认可用性 |
|--------|--------|------------|
| 证券主表 | mootdx 全市场代码 + 东财个股信息 | 当日 live；历史边界见 §20.1 |
| 交易日历 | 东财 / 交易所公开 | `PIT_REQUIRED` |
| 日线行情 | mootdx（主）+ 新浪 HTTP（备） | `PIT_REQUIRED` |
| 财务指标 | mootdx 财务快照 + 新浪三表 | `PIT_REQUIRED`（**须有可靠公告日**，见 §20.5） |
| 行业成员 | 东财板块当前成分 | `CURRENT_ONLY` |
| 概念成员 | 东财 / 百度当前归属 | `CURRENT_ONLY` |
| 指数成分 | 东财当前成分 | `CURRENT_ONLY` |
| 复权因子 | 待接入可靠免费源或可验证公司行为链 | 正式验收前阻塞（见 §20.3） |

#### 方案 B（可选）：Tushare Pro — `TushareProvider`

统一接口、申万行业 `in_date/out_date`、东财概念按交易日 `dc_member` 等。需积分与许可；**仅当** `TRADINGAGENTS_MARKET_DATA_PROVIDER=tushare` 且 Token 可用时启用。通过契约与 PIT 审计后可提升对应数据集等级。

#### 方案 C（未来）：商业授权源

Wind、聚宽等。阶段 4 只保留扩展点；任何源通过契约和 PIT 验证均可提升数据集等级，不限于 Tushare。

### 4.4 最终选择

阶段 4 采用 **`FreeAStockProvider 默认 + FixtureProvider 离线测试 + TushareProvider 可选增强`**。不得将多个来源静默拼接；每条数据必须记录来源和冲突处理。

### 4.5 数据能力矩阵（摘要）

完整矩阵见 `docs/data/data-capability-matrix.yaml`。摘要：

| 数据集 | 免费默认源 | 免费 PIT 等级 | 付费可增强 |
|--------|-----------|---------------|------------|
| 证券主表 | mootdx+东财 | `PIT_REQUIRED`（历史边界受快照覆盖约束） | `stock_basic` L/D/P |
| 交易日历 | 东财/交易所 | `PIT_REQUIRED` | `trade_cal` |
| 日线行情 | mootdx+新浪 | `PIT_REQUIRED` | `daily` |
| 财务数据 | mootdx+新浪 | `PIT_REQUIRED`（**无公告日不得入历史策略**） | `fina_indicator` |
| 申万行业成员 | 东财当前快照 | **`CURRENT_ONLY`** | `index_member_all` → `PIT_REQUIRED` |
| 指数成员 | 东财当前快照 | **`CURRENT_ONLY`** | 指数成分接口 |
| 东方财富概念 | 东财/百度当前快照 | **`CURRENT_ONLY`** | `dc_member` 历史快照 |
| 复权因子 | 待免费验证源 | 正式验收前须 `PIT_REQUIRED` | `adj_factor` |

Tushare 官方参考（可选增强路径核对用）：

- 数据目录：https://tushare.pro/document/2
- 申万行业成分：https://tushare.pro/document/2?doc_id=335

接口权限、积分和许可可能变化。部署前必须重新核对，并将核对日期写入能力矩阵。
...
└── providers/
    ├── base.py
    ├── fixture.py
    ├── free_astock.py    # 默认真实数据 Provider
    └── tushare.py        # 可选付费增强
...
### 任务 4.1：Provider 契约与默认免费适配器

- 实现 `FreeAStockProvider`（默认），封装 `a_stock.py` 已有直连能力；
- 保留 `TushareProvider` 为可选增强（`screener-tushare` 额外依赖）；
- 实现 Provider 工厂与 `TRADINGAGENTS_MARKET_DATA_PROVIDER` 解析；
- `FixtureProvider` 继续满足同一契约；
- 添加完全离线的契约测试；
- **禁止**将 Tushare 设为默认或隐式 probe。
...
1. 原单股分析入口和参数保持兼容。
2. 原 CLI 和 Web 正常启动。
3. **免费默认路径**：无网络额度、无 `TUSHARE_TOKEN` 时，`init`、同步、盘后调度可运行；fixture 模式正常。
4. **付费增强路径**（可选）：显式 `provider=tushare` + Token 时真实 Tushare 同步可验收。
5. 阶段 0–3 既有测试不得回归。
6. 新增单元、集成、端到端和 CLI 测试全部通过。
7. 测试默认不访问真实网络、不消耗真实额度。
8. Ruff 检查通过。
9. `.env`、Token、缓存、虚拟环境和本地数据库不得提交。

### 14.6 免费默认路径验收（必须通过）

1. `tradingagents-market-data init` 无 Token 成功。
2. `probe` / `sync`（security、calendar、daily、financials）无 Token 可运行。
3. `tradingagents-scheduler after-close` 无 Token 可运行（不调用 Tushare probe）。
4. 全市场、自定义 universe live 筛选可完成。
5. 行业 / 概念 / 指数 universe：**当日** live 可用；**历史 `as_of`** 返回 `data_error`。
6. 未定义板块代码返回 `data_error`，不得伪装 `empty_universe`。

### 14.7 正式历史策略验收（额外门槛，未满足则不得宣称阶段 4 历史回测通过）

1. 证券主表：目标 `as_of` 具备可靠历史证券快照；否则 `data_error`（不得用 `BEST_EFFORT` 冒充正式回测）。
2. 复权：可靠免费复权因子或可验证公司行为构造的 PIT 复权序列；动量因子 `price_basis=forward_adjusted`（或等价）；成交与涨跌停仍用未复权真实价格。
3. 财务：进入历史策略的记录必须有可靠 `announcement_date`（或 `actual_announcement_time`）。
4. 板块：仅当数据集经契约审计提升为 `PIT_REQUIRED` 后，历史板块 universe 方可通过。
5. 实验模式 `price_basis=raw` + `BEST_EFFORT` 可存在，但必须显著标记、不计入本节验收、不得作为正式回测结论输出。

## 15. 开发环境补充

在现有 MVP 环境基础上新增 DuckDB Python 包、可选 Parquet 支持库。

```text
默认（free）：init、同步、调度、fixture、单元测试 — 无需 TUSHARE_TOKEN
可选（tushare）：pip install -e ".[screener-tushare]" + TUSHARE_TOKEN + provider=tushare
```

导入模块时不得自动联网，测试不得依赖开发者个人 Token。`free` Provider 不得读取 `TUSHARE_TOKEN`。

## 16. 风险与降级

| 风险 | 处理方式 |
|---|---|
| 免费接口不稳定 | 缓存、限流、重试；失败 `ERROR`/`BLOCKED`，不转空池 |
| Tushare 积分不足 | 仅 `provider=tushare` 路径；缺权限阻止对应同步，不静默回退 free |
| 概念/行业/指数历史不完整 | 免费路径 `CURRENT_ONLY`；历史 universe → `data_error` |
| 证券历史退市边界缺失 | 正式历史回测 `data_error`；靠每日快照积累补全 |
| 复权因子未就绪 | 阻塞正式历史策略验收；允许实验 `price_basis=raw` 模式 |
| 财务无公告日 | 不得进入历史策略；拒绝入库或仅探索性 `BEST_EFFORT` |
...
**默认 `free` Provider** 在 probe 时只检测免费端点能力，不得调用 Tushare。

**`tushare` Provider** 仅在显式选择时 probe Tushare；不代表部署账号必然拥有全部接口。须记录每个接口的权限、历史起点、日期范围、频率限制、单次行数和许可声明。未通过探测的数据集不得标记为生产就绪，也不得被静默替换为空数据。

任何数据源（含 Tushare、聚宽、Wind 等）经契约测试和 PIT 审计通过后，可将对应数据集从 `CURRENT_ONLY` / `BEST_EFFORT` 提升为 `PIT_REQUIRED`。
...
**准入规则（强制）**：只有具备可靠 `announcement_date` 或 `actual_announcement_time` 的记录才允许标记为 `PIT_REQUIRED` 并进入历史策略。无公告日的财务记录不得进入正式历史回测；可拒绝入库或仅用于探索性 `BEST_EFFORT` 报告（须显著标记）。
...
## 20. 架构修订：免费默认与 PIT 边界（2026-06-19 用户确认）

本节纠正阶段 4 实现中将 Tushare 提升为默认主源的**需求漂移**，与 `2026-06-19-auto-stock-screening-design.md` 重新对齐。

### 20.1 证券主表与退市 PIT

- **当日实时选股**：允许使用当日有效证券池。
- **探索性历史报告**：快照覆盖不完整时可标记 `BEST_EFFORT` 并显著提示。
- **正式历史回测**：若目标 `as_of` 无可靠历史证券快照 → **`data_error`**；**不得**将 `BEST_EFFORT` 用于正式历史回测结论。
- **禁止**：从当前证券列表倒推历史股票池；在历史上提前暴露未来退市状态（`valid_to` / `delist_date` 的 `available_at` 不得早于市场可知时点）。
- **补全路径**：每日证券主表快照积累，或接入权威历史数据后，方可扩大可回测日期范围。

### 20.2 行业、概念与指数成分（免费路径）

- 免费源仅提供**当前快照** → `CURRENT_ONLY` + `membership_mode=current_only`。
- **当日** live 筛选：允许。
- **历史** `as_of` universe（`industry` / `concept` / `index`）→ **`data_error`**。
- 提升至 `PIT_REQUIRED`：须任意数据源通过契约与 PIT 审计（`effective_interval` 或 `dated_snapshot`），不限于 Tushare。

### 20.3 复权因子与动量基线

- **正式 V1 验收**：不接受未复权价格作为动量与历史回测基线。
- **成交与涨跌停**：始终使用未复权真实价格（与阶段 0–3 `limits.py` 一致）。
- **正式验收前**：须接入可靠免费复权因子，或基于可验证 `corporate_actions` 构造 PIT 复权序列。
- **实验模式**（可选，不计入正式验收）：
  - `price_basis=raw`
  - `pit_level=BEST_EFFORT`
  - 输出须显著标记「不适合正式绩效评估」
  - 不得作为阶段正式回测结论
- **不阻塞**：`FreeAStockProvider`、CLI、数据仓库开发可并行推进；阻塞的是「正式历史策略验收通过」的结论。
...
优先级：CLI --provider → 环境变量 → YAML market_data.provider → 默认 free
```

| 值 | 约束 |
|----|------|
| `free` | 不得读取 `TUSHARE_TOKEN` |
| `tushare` | 显式选择；缺 Token 立即报错，不自动回退 |
| `fixture` | 测试 / CI / 显式 `--fixture` |

### 20.5 财务 `PIT_REQUIRED` 收紧

仅当记录具备可靠 `announcement_date` 或 `actual_announcement_time` 时，方可标记 `PIT_REQUIRED` 并参与历史策略。缺失公告日的记录不得进入历史策略因子计算。
